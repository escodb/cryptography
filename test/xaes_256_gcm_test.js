...
const xaes256gcm = require('../lib/xaes_256_gcm')
const { assert } = require('chai')

describe('XAES-256-GCM', () => {
  it('encrypts when MSB = 0', async () => {
    ...
    let ct = await xaes256gcm.encrypt(key, iv, msg)
    let pt = await xaes256gcm.decrypt(key, iv, ct)
  ...
  it('encrypts with AAD when MSB = 1', async () => {
    ...
    let ct = await xaes256gcm.encrypt(key, iv, msg, aad)
    let pt = await xaes256gcm.decrypt(key, iv, ct, aad)
