...
module.exports = {
  ...
  hkdfSha256: {
    async derive (key, salt, info, size) {
      key = await subtle.importKey('raw', key, 'HKDF', false, ['deriveKey'])

      let params = { name: 'HKDF', hash: 'SHA-256', salt, info }
      let algo = { name: 'AES-GCM', length: size }
      let derived = await subtle.deriveKey(params, key, algo, true, ['encrypt'])

      derived = await subtle.exportKey('raw', derived)
  ...
  aes256: {
    async encrypt (key, data) {
      let params = { name: 'AES-CBC', length: AES_KEY_SIZE }
      key = await subtle.importKey('raw', key, params, false, ['encrypt'])

      let iv = Buffer.alloc(AES_BLOCK_SIZE / 8).fill(0)
      params = { ...params, iv }
      let enc = await subtle.encrypt(params, key, data)

      return Buffer.from(enc).subarray(0, AES_BLOCK_SIZE / 8)
    }
  },
